		...
		switch (Ext.type(predicate)) {
			...
			default :
				throw ['Predicate: ', predicate, ' is not callable in "',
						this.current_state, '" state for event "', event].join('');
		}
		return result;
	},
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
	...
	initComponent : function() {
		...
		var tt = {
			...
			'ready' : {
				'file-selected' : [{predicate : [this.fireFileTestEvent,
	...
	fillToolbar : function() {
		...
		if (this.isView == false) {
			...
			tb.x_buttons.indicator = tb
					.addItem(new Ext.Toolbar.Item(Ext.DomHelper.append(tb.getEl(), {
	...
	updateProgressBar : function() {
		if (this.is_uploading) {
			...
			this.progress_bar.updateProgress(value, String.format(this.i18n.progress_uploading_text,
						...
						if (!Ext.isEmpty(delIds)) {
							Ext.Ajax.request({
										...
										success : function(result, request) {
											Ext.Msg.alert('', '删除附件成功！');
										},
										failure : function(result, request) {
											Ext.Msg.alert('', '删除附件失败！');
										},
										params : {
											fileIds : delIds
										}
